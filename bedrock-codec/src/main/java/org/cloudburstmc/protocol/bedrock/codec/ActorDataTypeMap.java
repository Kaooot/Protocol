...
public class ActorDataTypeMap {

    private static final ActorDataFormat[] FORMATS = ActorDataFormat.values();

    private final Definition<?>[][][] idDefinitions;
    private final Map<ActorDataType<?>, Definition<?>> typeDefinitionMap;

    public Definition<?>[] fromId(int id, ActorDataFormat format) {
    ...
    public <T> Definition<T> fromType(ActorDataType<T> type) {
    ...
    public String prettyPrint() {
        TreeMap<Integer, ActorDataType<?>> map = new TreeMap<>();
        this.typeDefinitionMap.forEach((type, def) -> map.put(def.id, type));


        StringJoiner joiner = new StringJoiner("\n");
        for (Map.Entry<Integer, ActorDataType<?>> entry : map.entrySet()) {
    ...
    public static class Definition<T> {
        int id;
        final ActorDataType<T> type;
        ActorDataTransformer<?, T> transformer;
        final ActorDataFormat format;
    ...
    public static class Builder {
        ...
        private final Map<ActorDataType<?>, Definition<?>> typeDefinitionMap = new IdentityHashMap<>();
        ...
        public <T> Builder replace(ActorDataType<T> type, int index, ActorDataFormat format) {
            return replace(type, index, format, ActorDataTransformer.identity());
        }
         ...
         * @return this builder
         */
        public <T> Builder replace(ActorDataType<T> type, int index, ActorDataFormat format, ActorDataTransformer<?, T> transformer) {
        ...
        public <T> Builder insert(ActorDataType<T> type, int index, ActorDataFormat format) {
            return insert(type, index, format, ActorDataTransformer.identity());
        }
        ...
        @SuppressWarnings("unchecked")
        public <T> Builder update(ActorDataType<T> type, ActorDataTransformer<?, T> transformer) {
            checkNotNull(type, "type");
            checkNotNull(transformer, "transformer");
        ...
        public ActorDataTypeMap build() {
            return new ActorDataTypeMap(copy(this.types), this.typeDefinitionMap);
        }
    }
}
